...
from rewards import lanes_locations, get_desired_lane_index, lane_index_to_lane_boundary
...
def animate_trajectory(traj):
    ...
    time = 0
    desired_lane_index = get_desired_lane_index(time)
    boundaries = lane_index_to_lane_boundary(desired_lane_index)
    desired_lane = [0,0]
    desired_lane[0] = ax.axhline( y = boundaries[0], color='r' )
    desired_lane[1] = ax.axhline( y = boundaries[1], color='r' )
    dt = 0.05
    for i in range(traj.shape[1]):
        ...
        desired_lane[0].remove()
        desired_lane[0] = ax.axhline(y=boundaries[0], color='r')
        desired_lane[1].remove()
        desired_lane[1] = ax.axhline(y=boundaries[1], color='r')
        fig.canvas.draw()
